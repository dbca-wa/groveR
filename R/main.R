...
#'    created by using "\code{link{cloud_mask}}.
#'
#'    The function will perform some "probable" values classification in those
...
#'     which works with the suggested project folder structure and workflow.
#'
#' @return All input rasters will be reclassified and be written to `veg_class/`.
#'
#' @author Bart Huntley, \email{bart.huntley@@dbca.wa.gov.au}
#'
#' @examples
...
#' @import cli
#' @importFrom fs dir_ls file_copy
#' @importFrom stringr str_replace str_split_i
...
veg_class <- function(irast, ext = ".tif", classes = "supplementary/density_classes.csv"){
  ...
  cl <- as.matrix(readr::read_csv(classes, col_types = readr::cols()))
  if(dim(cl)[2] !=3) cli::cli_abort(c("Classes csv must contain 3 columns",
                                      "i" = "Double check format of the csv"))
  # make df of IO names
  rastdf <- dplyr::tibble(path = fs::dir_ls(irast, glob = paste0("*", ext, "$")))
  # finds correct pattern to change output name if from different source
  npat <- stringr::str_split_i(basename(rastdf[[1]]), "_", 2)[1]
  # finds correct pattern to change output path if from different source
  dpat <- dirname(rastdf[[1]])[1]
  # change outname appropriately and get year
  rastdf <- rastdf |>
    dplyr::mutate(outname = stringr::str_replace(path, npat, "vegclass"),
                  outname = stringr::str_replace(outname, dpat, "veg_class"),
                  yr = readr::parse_number(basename(outname))) |>
    dplyr::arrange(yr)
  # output folder
  out <- "./veg_class"
  if(!file.exists(out)){dir.create(out)}
  # function to handle cloud probability
  cldfun = function(x1, x2){
    ifelse(is.na(x1) & is.na(x2), NA,
           ifelse(x2 == 6 & is.na(x1), x1,
                  ifelse(x2 == 6 & x1 == 6, 6,
                         ifelse(x2 == 6 & x1 != 6, x1 + 10, x2))))}
  # handle special case where 1st year has cloud (no prior image for probability)
  # by classifying 1st year by self
  yr1 <- terra::rast(rastdf[[1]][1])
  rc1 <- terra::classify(yr1, cl)
  terra::writeRaster(x = rc1, filename = rastdf[["outname"]][1], datatype = 'INT1U',
                     overwrite=TRUE)
  for(i in 2:length(rastdf[[1]])){
    rst <- terra::rast(rastdf[[1]][i])
    rc <- terra::classify(rst, cl)
    if(terra::minmax(rc)[2] == 6){
      pre <- terra::rast(rastdf[["outname"]][i - 1])
      stk <- c(pre, rc)
      rc <- terra::lapp(stk, fun = cldfun)
    }
    terra::writeRaster(x = rc, filename = rastdf[["outname"]][i], datatype = 'INT1U',
                       overwrite=TRUE)
  }
}
...
#' @importFrom readr write_csv parse_number
#'
#' @export
veg_class_area <- function(irast, iregions, attribname, areaname, ext = ".tif",
  ...
  rastdf <- dplyr::tibble(path = fs::dir_ls(irast, glob = paste0("*", ext, "$"))) |>
    dplyr::mutate(yr = readr::parse_number(basename(path))) |>
    dplyr::arrange(yr)
  # determine reporting regions
  reps <- unique(regions[[attribname]][[1]])
  # output folder
  out <- "./extent_summaries"
  if (!file.exists(out)) {dir.create(out)}
  # read all rasters into a stack
  rst <- terra::rast(rastdf[[1]])
  # find pixel res and calculate hectares
  res_mult <- (round(terra::res(rst)[1])^2)/10000
  stats <- dplyr::tibble()
  cli::cli_alert_info("Calculating vegetation classification areas")

  for(i in seq_along(reps)){
    rep_i <- regions[i, attribname]
    name_r <- stringr::str_split(reps[i], "_")[[1]][1]
    name_s <- stringr::str_split(reps[i], "_")[[1]][2]
    cli::cli_alert_info("Working on {.emph {name_r}} {.emph {name_s}}")
    # make raster mask
    rep_ir <- terra::rasterize(x = rep_i, y = rst[[1]])
    # mask out reporting region
    msk_ir <- terra::mask(rst, rep_ir)
    names(msk_ir) <- rastdf[[2]]
    # calc freq
    stk <- terra::freq(msk_ir, usenames = TRUE)
    if(isTRUE(probabilities)){
      # include cloud probabilities
      out_df <- stk|>
      ...
      # ignore cloud probabilities and just report as cloud
      out_df <- stk|>
        dplyr::mutate(Region = name_r,
                      Site = name_s,
                      Area = count * res_mult,
  ...
  # find start end year
  yrs <- paste0("_", min(rastdf[[2]]), "-", max(rastdf[[2]]), "_")
  # output name
...
#'
#' @export
trend_class <- function(irast, areaname, end, period = 10,
                        classes = "supplementary/trend_classes.csv", ext = ".tif"){
  cli::cli_alert_info("Calculating vegetation classification trends")
  # make df for IO
  rastdf <- dplyr::tibble(path = fs::dir_ls(irast, glob = paste0("*", ext, "$"))) |>
    dplyr::mutate(yr = readr::parse_number(basename(path)))
  ...
  expectdf <- dplyr::tibble(yr = true_period)|>
    dplyr::mutate(path = stringr::str_replace(gstring, "XX", as.character(yr)))
  # df of missing rasters if any
  dum_rst <- dplyr::setdiff(expectdf, rastdf)
  # empty dummy raster (required to pad timeseries out for linear trend)
  blank <- terra::rast(rstring)
  terra::values(blank) <- NA
  # write out dummy rasters if required
  if(nrow(dum_rst) > 0){
    cli::cli_alert_warning("Found missing years in time series")
    cli::cli_alert_info("Creating dummy years")
    for(i in seq_along(dum_rst[[2]])){
      nname <- dum_rst[[2]][i]
      terra::writeRaster(blank, nname, datatype = 'INT1U', overwrite = TRUE)
    }
  } else {
    cli::cli_alert_info("No missing years found")
  }

  # make stack and sensible names
  stk <- terra::rast(expectdf[["path"]])
  s_layer_names <- expectdf[["yr"]]
  names(stk) <- s_layer_names
  # subset stack to trend period
  stk_e <- grep(pattern = end, names(stk))
  stk_s <- stk_e - (period - 1)
  trnd_stk <- stk[[stk_s:stk_e]]
  # recode -99 clouds to NAs for trend to make sense
  trnd_stk<- terra::subst(trnd_stk, -99, NA)
  # output folder
  out <- "./trend_class"
  if (!file.exists(out)) {dir.create(out)}
  # per pixel lm
  ...
  # get classes
  cl <- as.matrix(readr::read_csv(classes, show_col_types = FALSE))
  ...
  oname <- paste0(out, "/", areaname, yrs, "trendclass", ext)
  tname <- paste0(out, "/", areaname, yrs, "trend", ext)
  terra::writeRaster(trnd_cl, filename = oname, datatype = 'INT1U',
                     overwrite = TRUE)
  terra::writeRaster(trend, filename = tname, datatype = 'FLT4S',
                     overwrite = TRUE)
  # removal of dummy years if applicable
  if(nrow(dum_rst) > 0){
    cli::cli_alert_info("Removing dummy years")
    for(i in seq_along(dum_rst[[2]])){
